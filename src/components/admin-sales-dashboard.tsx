...
function SummaryCard({ detail, label, value }: { detail: string; label: string; value: string | number }) {
  return (
    <div className="rounded-2xl border border-[#dfddd4] bg-white p-4 shadow-[0_12px_35px_rgba(0,0,0,0.04)] sm:p-5">
...
function FilterControls({ selection }: { selection: SalesFilterSelection }) {
  ...
  return (
    <section className="rounded-[1.5rem] border border-[#dfddd4] bg-[#f7f6f1] p-4 sm:p-7">
      ...
      <form action="/admin/sales" aria-describedby={selection.error ? "sales-range-error" : undefined} className="mt-4 flex flex-col gap-3 rounded-2xl border border-[#dfddd4] bg-white p-3 sm:flex-row sm:flex-wrap sm:items-end sm:p-4" method="get">
...
function StaffEarningsSummary({ items }: { items: SalesReport["staffEarnings"] }) {
  if (items.length === 0) {
    return <p className="rounded-xl border border-dashed border-[#cfcac0] bg-white p-5 text-sm leading-6 text-[#65635d]">No staff earnings recorded for this period.</p>;
  }
  ...
  return (
    <header className="flex flex-col gap-4 sm:gap-6 md:flex-row md:items-end md:justify-between">
      <div className="max-w-3xl">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[#65635d] sm:mt-4 sm:text-base sm:leading-7">Sales activity is recognized at completion and uses the historical transaction snapshots stored at check-in.</p>
      </div>
      <div className="flex items-center gap-3 self-start md:self-auto">
        <Link className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#d7d4ca] bg-white px-4 text-sm font-bold text-[#292929] shadow-sm transition-colors hover:border-[#c7a900] hover:bg-[#fff7cc]" href="/admin/sales/staff">
          <Users className="h-4 w-4 text-[#a77f00]" />
          Manage staff
        </Link>
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#fff7cc] text-[#a77f00] ring-1 ring-[#ead98a] sm:h-16 sm:w-16">
          <ChartLine className="h-7 w-7 sm:h-8 sm:w-8" />
        </div>
      </div>
    </header>
  );
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[1.5rem] border border-[#dfddd4] bg-[#f7f6f1] p-4 sm:p-7">
      ...
      <section className="rounded-[1.5rem] border border-[#dfddd4] bg-[#f7f6f1] p-4 sm:p-7">
        ...
        <div className="mt-5"><StaffEarningsSummary items={report.staffEarnings} /></div>
      </section>
