...
export default function CheckInError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7f6f1] px-4 py-10">
      <section className="w-full max-w-lg rounded-[1.75rem] border border-[#f0d3c8] bg-white p-6 text-center shadow-[0_20px_55px_rgba(0,0,0,0.08)] sm:p-10">
        ...
        <button className="mt-7 inline-flex min-h-13 items-center justify-center rounded-xl bg-[#f4c400] px-6 text-sm font-bold text-[#171717] transition-colors hover:bg-[#d8aa00]" onClick={reset} type="button">
          Try again
        </button>
      </section>
    </main>
