-- Phase 8: admin-only sales reporting from completed transaction snapshots.

create index transactions_completed_at_sales_idx
  on public.transactions (completed_at desc)
  where status = 'completed'::public.transaction_status;

-- Return all sales-reporting aggregates and the selected completed transaction
-- snapshots in one protected database query. Date arguments are Manila calendar
-- dates; the function converts their boundaries to timestamptz consistently.
...
comment on function public.get_admin_sales_report(date, date) is
  'Returns completed-transaction sales summaries and historical snapshot breakdowns for the active admin.';
